...
pnpm --filter=prettier-plugin-apex run debug-check "$location/$name/**/{classes,triggers}/*.{cls,trigger}" --no-config --ignore-path=./tests_integration/format/ignore-list.txt
rm -rf "$location/$name"
