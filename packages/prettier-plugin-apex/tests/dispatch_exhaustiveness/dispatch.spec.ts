...
const UNHANDLED_CLASSES = new Set<string>([
  ...
  // PRE-EXISTING GAP (not introduced by this refactor): the SOQL `WITH`
  // tuple form. `genericPrint` would throw "No handler found" if a query used
  // it. No fixture exercises it today. Tracked as currently-unsupported grammar;
  // remove from this list when a handler + fixtures are added.
