...
- Each test folder has a `jsfmt.spec.js` that runs the tests. Generally you can just put `run_spec(__dirname, ["apex"]);` there. This will verify that the output using the Apex parser stays consistent. You can also pass options as the third argument, like this: `run_spec(__dirname, ["apex"], { apexAnonymous: true });`
